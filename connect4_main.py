...
def main():
    ...
    # Create the window
    window_width = connect4_board.WIDTH * connect4_board.TILE_SIZE
    window_height = connect4_board.HEIGHT * connect4_board.TILE_SIZE
    window_size = (window_width, window_height)
    screen = pygame.display.set_mode(window_size)

    # Create the game board
    the_board = connect4_board.Board()
    game_over = False
        ...
        if the_board.current_player == 1:
            ...
            while not played_move:
                ...
                if event.type == pygame.QUIT:
                    ...
                    square_x = mouse_x / connect4_board.TILE_SIZE
                    if the_board.do_move(square_x):
                        played_move = True
                    else:
                        print("Illegal move")
        else:
            ...
            the_board.do_move(move)
        
        # Check if game is over
        result = the_board.get_result(1)
        if result is not None:
            ...
            if the_board.winning_line is not None:
                ...
                pygame.draw.line(screen, (100, 100, 200),
                                 ((x + 0.5) * connect4_board.TILE_SIZE, (y + 0.5) * connect4_board.TILE_SIZE),
                                 ((x + 0.5 + dx*3) * connect4_board.TILE_SIZE, (y + 0.5 + dy * 3) * connect4_board.TILE_SIZE),
                                 5)
